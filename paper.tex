...
Three further reductions are unconditional and may help.  Running the induction
along the socle steps, with $D$ the colour-$t$ threshold of the partial module
$X$: (a) a colour-$t$ step contributes nothing in degrees $<D-2$; (b) if it
contributes at $D-2$ then $\dim X_t[D]=1$ and the $D-2$ contribution is exactly
one special line, so a single further dimension at $D$ suffices.  Both follow
from the simple socle of $I_\ka$ together with the propagation of Lemma
\ref{lem:Id}.  And (c), the condition ``every neighbour of $t$ is full at degree $D+1$'' ---
which is what makes a colour-$t$ socle step reach the threshold $D$ --- is
provable at \emph{all three} neighbours, assuming only that $X$ is an up-set at
those neighbours.  At a colour-$t$ step $c=\mu'_t-\mu'_{n-1}\ge1$ with
$\mu'_t\le1$ forces $\mu'_{n-1}\le0$, so $r'=|\mathrm{supp}\,\mu'\cap\{n-1,n\}|$
is $0$ or $2$; and if $X_t[D]\ne0$ then $v'_t=2a$ is even with $D=t+k-2a$.

At the leaves, the $\varepsilon$-coordinate identities give
\[
   v'_{n-1}+v'_n=v'_t-\mu'_{n-1}\in\{2a,2a+1\},\qquad
   |v'_{n-1}-v'_n|=|\mu'_n|\le1,
\]
so $\min(v'_{n-1},v'_n)\ge a$, which is exactly the condition for an up-set at
a leaf to be full at $D+1$.  At the chain neighbour, $c\ge1$ gives
$v'_{t-1}\ge2a-[k=t]$, while the profile of $I_\ka$ satisfies
\[
   \sum_{d\ge D+1}\dim (I_\ka)_{t-1}[d]\ \le\ 2a-[k=t]
\]
(checked for all $D_5$--$D_8$ and all $k\le n-2$, $52/52$; it is a statement
about the image formula of Lemma \ref{lem:P} alone), so an up-set at $t-1$ is
full at $D+1$ as well.  Both halves are confirmed directly, at $1104$ leaf
instances and $527$ chain instances.

So the conjecture reduces to the up-set property at $t$ and at its neighbours
holding \emph{simultaneously} along the construction --- which is why we state
it as one statement about staircases rather than as a chain of separate claims.

Two facts delimit the search.Two facts delimit the search.  The conjecture genuinely needs extremality: the
